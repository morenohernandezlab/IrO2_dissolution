%% This script will calculate the steady-state concentration of radiolysis products from initial concentrations of Fe(III) and Cl-
...
line 23-26 set the ratios of Fe and Cl
line 30 set concentrations of Fe and Cl
line 31 sets the pH
%}

clc
tol_used = 10^-8; %Model tolerance, change to 10^-11 for full resolution
time_run = 1000; %s
beam_dose = 2000;
...
for i = 1:size(Fe_Cl_ratios,1)
    i;
salt = 100; %mM
pH_initial = 1;
H2O_initial = 55.6; %M
Cl_initial = Fe_Cl_ratios(i,2)*salt*10^-3; %M
Fe2_initial = 0;
Fe3_initial = Fe_Cl_ratios(i,1)*salt*10^-3;  %M
Fe4_initial = 0;
Fe5_initial = 0;
Fe6_initial = 0;
...
conservation_matrix(20,:) = [-1, 0, 0, 2, 0]; %Cl2_mono_ions  = reactants(20);
...
conservation_matrix(26,:) = [0, 3, 4, 0, 1]; %Fe5_ions  = reactants(26);
...
end_concentrations = interp1(times,concentrations,time_interp);

radio_info(i).experiment = i;
radio_info(i).settings = [Fe_Cl_ratios(i,1), Fe_Cl_ratios(i,2)].*salt; %mM
radio_info(i).start_concentrations = initialConcentration;
radio_info(i).time = time_interp;
radio_info(i).end_concentration = end_concentrations;
end
%%
%save radio_info_water_Fe_Cl_2024_07_25 radio_info
%%
% Compute conservation
%
concentrations_elements = concentrations*conservation_matrix;
elements_error = concentrations_elements(end,:)-concentrations_elements(1,:);
...
average_oxidation = (concentrations(:,23)*2+concentrations(:,24)*3+concentrations(:,25)*4+concentrations(:,26)*5+concentrations(:,27)*6)./(concentrations(:,23)+concentrations(:,24)+concentrations(:,25)+concentrations(:,26)+concentrations(:,27));
clf
plot(times,average_oxidation)
%
% Plotting
clf
line_width = 5;
